...
def inference(VEPM_Models, DS, args):
    ...
    phi, _, _ = VEPM_Models.INFnet(adj, x)
...
def train(VEPM_Models, VEPM_Optims, DS, epoch, args):
    t0 = time.time()
    VEPM_Models.INFnet.eval()
    VEPM_Models.GENnet.train()

    yhat = inference(VEPM_Models, DS, args)
...
if __name__ == '__main__':
    ...
    # initialize Logger
    logdir = os.path.join(args.logdir, f'pt-{args.Epch_loadmodel}-epch')
    ...
    for epoch in range(args.Epch_finetune+1):
        train(VEPM_Models, VEPM_Optims, DS, epoch, args)
